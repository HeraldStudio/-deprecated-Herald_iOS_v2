...
class CardView: UIView {
    ...
    private func setupSubviews() {
        ...
        // 容器View
        containerView_1.into(self).below(staticLabel,25).left(70).right(70).height(30)
        
        // 余额
        balanceLabel.into(containerView_1).top(0).bottom(0).left(0).width(130).background(HeraldColorHelper.PrimaryBg).font(17,.semibold).align(.center)
        
        // 充值
        topUpButton.into(containerView_1).top(0).bottom(0).right(0).background(HeraldColorHelper.PrimaryBg)
        let textAttrString = NSMutableAttributedString.init(string: "充值")
        textAttrString.font(17, FontWeight.semibold, NSMakeRange(0, 2))
        topUpButton.setAttributedTitle(textAttrString, for: .normal)
        
        // 消费次数
        comsumeTimesLabel.into(containerView_1).top(0).bottom(0).after(balanceLabel, 5).before(topUpButton,5).background(HeraldColorHelper.PrimaryBg).font(17,.semibold).align(.center)
        ...
        // 容器View
        containerView_2.into(self).below(cardTableView,10).left(70).right(70).height(30).bottom(0)
        
        // 总支出
        totalCostlabel.into(containerView_2).top(0).left(0).bottom(0).width(200).background(UIColor.white).font(17,.semibold).align(.center).color(HeraldColorHelper.Primary)
        
        // 加载前一天
        loadButton.into(containerView_2).top(0).right(0).bottom(0).width(150).background(HeraldColorHelper.PrimaryBg)
        let yet_textAttrString = NSMutableAttributedString.init(string: "加载前一天")
        yet_textAttrString.font(17, FontWeight.semibold, NSMakeRange(0, 5))
