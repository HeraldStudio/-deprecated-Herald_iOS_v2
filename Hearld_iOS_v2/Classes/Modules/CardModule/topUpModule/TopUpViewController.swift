...
class TopUpViewController: UIViewController {
    ...
    private func layoutUI() {
        ...
        if let navigationController = self.navigationController as? MainNavigationController {
            // 文字
            textView.into(view).top(navigationController.getHeight() + 70).left(30).right(30).text("一卡通在线预充值并非实时到账，在校多数食堂超市网点刷卡时均可到账；极少数情况不能到账时，你也可以手动在圈存机存入任意金额；本功能仅在银行卡与学校一卡通中心之间产生交易，与小猴偷米无关")
            
            // 容器视图
            containerView.into(view).below(textView,20).left(30).right(30).height(50)
            ...
            // 选择数值展示框
            displayTextField.into(view).below(containerView,25).width(200).centerX().text("50")
            
            // 密码输入框
            passwordTextField.into(view).below(containerView,8).width(200).centerX()
            
            // 确认按钮
            confirmButton.into(view).below(passwordTextField,8).width(120).bottom(5).centerX().background(HeraldColorHelper.PrimaryBg)
